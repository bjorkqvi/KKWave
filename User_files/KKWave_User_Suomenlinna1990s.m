function [Info,Site,Forcing,Bnd,Bnd_Forcing,Nesting] = KKWave_User_Suomenlinna1990s
% function [Forcing] = KKWave_User_GoF

%% General info about the run
Info.folder=pwd; % Project folder
Info.name='Suomenlinna1990s'; % Project name
...
Forcing.wind_file=sprintf('%s/wind/Wind_Harmaja_1h_1990s.asc',Info.folder); % Wind data
Forcing.ta_file=sprintf('%s/Ta/Ta_Harmaja_1h_1990s.asc',Info.folder); % Air temperature data
Forcing.tw_file=sprintf('%s/Tw/Tw_LL7mean_and_Harmaja_1h_1990s.asc',Info.folder); % Water temperature data



%% Boundary data
Info.BoundaryInput=sprintf('%s/bnd/BND_GoF1990s.mat',Info.folder);
