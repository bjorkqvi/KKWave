function [Info,Site,Forcing,Bnd,Bnd_Forcing,Nesting] = KKWave_User_Suomenlinna2010s
% function [Forcing] = KKWave_User_GoF

%% General info about the run
Info.folder=pwd; % Project folder
Info.name='Suomenlinna2010s'; % Project name
...
Forcing.wind_file=sprintf('%s/wind/Wind_Harmaja_1h_2010s.asc',Info.folder); % Wind data
Forcing.ta_file=sprintf('%s/Ta/Ta_Harmaja_1h_2010s.asc',Info.folder); % Air temperature data
Forcing.tw_file=sprintf('%s/Tw/Tw_Harmaja_1h_2010s.asc',Info.folder); % Water temperature data



%% Boundary data
Info.BoundaryInput=sprintf('%s/bnd/BND_GoF2010s.mat',Info.folder);
