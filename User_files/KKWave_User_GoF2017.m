function [Info,Site,Forcing,Bnd,Bnd_Forcing,Nesting] = KKWave_User_GoF2017
% function [Forcing] = KKWave_User_GoF

%% General info about the run
Info.folder=pwd; % Project folder
Info.name='GoF2017'; % Project name
...
Forcing.wind_file=sprintf('%s/wind/Wind_Kalbadagrund_1h_2017.asc',Info.folder); % Wind data
Forcing.ta_file=sprintf('%s/Ta/Ta_Kalbadagrund_1h_2017.asc',Info.folder); % Air temperature data
Forcing.tw_file=sprintf('%s/Tw/Tw_LL7mean_1h_2017.asc',Info.folder); % Water temperature data
...
if ~isfield('Info','BoundaryInput') | ~Info.BoundaryInput
    ...
    Bnd_Forcing(1).wind_file=sprintf('%s/wind/Wind_Kalbadagrund_1h_2017.asc',Info.folder); % Wind data
    Bnd_Forcing(1).ta_file=sprintf('%s/Ta/Ta_Kalbadagrund_1h_2017.asc',Info.folder); % Air temperature data
    Bnd_Forcing(1).tw_file=sprintf('%s/Tw/Tw_LL7mean_1h_2017.asc',Info.folder); % Water temperature data
    ...
    Bnd(2).wind_att=0.8;
    Bnd_Forcing(2).wind_file=sprintf('%s/wind/Wind_Uto_1h_2017.asc',Info.folder); % Wind data
    Bnd_Forcing(2).ta_file=sprintf('%s/Ta/Ta_Uto_1h_2017.asc',Info.folder); % Air temperature data
    Bnd_Forcing(2).tw_file=sprintf('%s/Tw/Tw_LL7mean_1h_2017.asc',Info.folder); % Water temperature data
else
