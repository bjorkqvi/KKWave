function [Info,Site,Forcing,Bnd,Bnd_Forcing,Nesting] = KKWave_User_Suomenlinna2016
% function [Info,Site,Forcing,Bnd,Bnd_Forcing,Nesting] = KKWave_User_Suomenlinna2016

%% General info about the run
Info.folder=pwd; % Project folder
Info.name='Suomenlinna2016'; % Project name
...
Forcing.wind_file=sprintf('%s/wind/Wind_Harmaja_1h_2016.asc',Info.folder); % Wind data
Forcing.ta_file=sprintf('%s/Ta/Ta_Harmaja_1h_2016.asc',Info.folder); % Air temperature data
Forcing.tw_file=sprintf('%s/Tw/Tw_Harmaja_1h_2016.asc',Info.folder); % Water temperature data



%% Boundary data
Info.BoundaryInput=sprintf('%s/bnd/BND_GoF2016.mat',Info.folder);
